  ...
  for (let i = 0; i < tb_len; i++) {
    ...
    for (let key in row) {
      if (re.test(row[key])) { // 如果为图片，则需要加div包住图片
        // 
        tbody += '<td style="width:'+width+ 'px; height:' + height + 'px; text-align: center; vertical-align: middle"><div style="display:inline;margin:3px"><img src=\'' + row[key] + '\' ' + ' ' + 'width=' + '\"' + (width-6) + '\"' + ' ' + 'height=' + '\"' + (height-6) + '\"' + '></div></td>'
      } else {
